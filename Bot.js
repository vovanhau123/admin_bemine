...
client.once("ready", async () => {
  ...
  try {
    ...
    await rest.put(
      Routes.applicationGuildCommands(client.user.id, "1232267648117641266"), // Thay 'YOUR_GUILD_ID' bằng ID của server
  ...
  if (interaction.commandName === "chat") {
    ...
    try {
      ...
      await interaction.reply({
        content: "Tin nhắn gửi thành công",
        ephemeral: true,
      });
    } catch (error) {
      console.error(error);
      await interaction.reply({
        content: "Có lỗi xảy ra khi gửi embed.",
        ephemeral: true,
      });
    }
  }
